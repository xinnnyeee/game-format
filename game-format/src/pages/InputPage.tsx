...
export default function InputPage() {
  ...
  // Function to show banner with message
  const showErrorBanner = (
    message: string,
    type: "warning" | "error" | "info" | "success" = "error"
  ) => {
    setBannerMessage(message);
    setBannerType(type);
    setShowBanner(true);
    console.log("Showing banner:", message); // Debug log
  };
  ...
  const hideBanner = () => {
    setShowBanner(false);
    setBannerMessage("");
    console.log("Hiding banner"); // Debug log
  };
  ...
  const handleCourtInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value > Math.floor(players.length / 2)) {
      showErrorBanner(
        "Number of courts is too many for the current number of players"
      );
    } else {
      hideBanner(); // Hide banner if the error is resolved
    }
  ...
  const enterGame = () => {
    // Check for KING OF THE COURT specific validation
    if (selectedFormat === "KING OF THE COURT") {
      const filledPlayers = players.filter(
        (player) => player.trim() !== ""
      ).length;
      if (numOfCourts * 4 >= filledPlayers) {
        showErrorBanner(
          `Too few players for the number of court. You need at least ${
            numOfCourts * 4 + 1
          }
           players for ${numOfCourts} courts.`
        );
        return;
      }
    }
  ...
  return (
    <>
      ...
      <div
        className={`min-h-screen px-8 py-6 font-sans bg-white flex flex-col ${
          showBanner ? "pt-16" : ""
        }`}
      >
        {/* Header */}
